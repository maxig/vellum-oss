...
EOF
    ...
    info "No wildcard cert. $APP_DOMAIN and any career-site host with its own DNS"
    info "record still get certs individually, issued on first request."
  fi
else
...
# Generated by setup.sh
{
	...
	# Customer domains (careers.acme.com) get their certificate the first time
	# a browser asks for one. Caddy checks with the app first, so a cert is only
	# ever issued for a hostname some workspace has actually saved in
	# Settings → Career site. Never remove this ask: without it, anyone could
	# point DNS at this box and burn the Let's Encrypt rate limit.
	on_demand_tls {
...
# Anything else: a workspace's own domain. This block is why adding a company
# needs no Caddyfile edit — the ask endpoint above decides which hosts are real.
https:// {
	encode zstd gzip
	tls {
		on_demand
...
# Generated by setup.sh
{
	...
	# See the wildcard variant of this file for the full note: a cert is only
	# issued for a hostname a workspace has saved in Settings → Career site.
	on_demand_tls {
		ask http://app:3000/api/public/domain-check
	}
}
...
# Workspace career sites reachable by their own domain (careers.acme.com).
# Without the wildcard cert above, <slug>.<apex> subdomains land here too and
# get an individual certificate — which works, as long as each subdomain has
# its own DNS record.
https:// {
	encode zstd gzip
	tls {
...
EOF
    ok "Caddyfile + Dockerfile.caddy written (app domain + on-demand custom domains)."
  fi
fi
