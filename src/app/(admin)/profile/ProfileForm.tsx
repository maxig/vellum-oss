...
export default function ProfileForm({ user, role, workspaceName }: { user: { id: string; name: string; email: string }; role: string; workspaceName: string }) {
  ...
  async function save() {
    setSaving(true);
    await fetch("/api/profile", { method: "PATCH", body: JSON.stringify({ name, password: password || undefined }) });
    setSaving(false);
    setSavedAt(Date.now());
    setPassword("");
  }
