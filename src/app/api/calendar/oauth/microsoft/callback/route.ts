...
export async function GET(req: Request) {
  ...
  const expected = c.get("vellum_oauth_microsoft")?.value;
  c.delete("vellum_oauth_microsoft");
  ...
  try {
    ...
    await db.calendarAccount.upsert({
      where: {
        userId_provider_email: { userId: user.id, provider: "microsoft", email: tokens.email },
      },
      create: {
