 ...
 *
 * Startup is logged at info; every tick logs at debug, so `LOG_LEVEL=debug` plus
 * a grep for `[email-worker]` / `[email-poll]` in the server log follows what's
...
async function tick() {
  ...
  try {
    ...
    log.debug(
      `tick #${tickId} starting at ${new Date(startedAt).toISOString()} · ${accounts.length} enabled account${accounts.length === 1 ? "" : "s"}`,
    );
    if (accounts.length === 0) {
      log.debug("tick #" + tickId + " no enabled email accounts");
    }
    for (const acct of accounts) {
      try {
        ...
        await db.emailAccount
          ...
          .catch(() => {});
        log.debug(
          `tick #${tickId} ${acct.workspaceId} OK · ingested=${result.ingested} checked=${result.checked} since=${result.since}`,
        );
      } catch (e) {
        const msg = (e as Error).message;
        log.warn(`tick #${tickId} ${acct.workspaceId} FAILED:`, msg);
    ...
    log.debug(`tick #${tickId} finished in ${elapsed}ms`);
  }
}

export function startEmailWorker() {
  if (process.env.EMAIL_POLL_DISABLED === "1") {
    log.info("disabled by EMAIL_POLL_DISABLED=1");
    return;
  }
  const s = state();
  if (s.started) {
  ...
  s.started = true;
  log.info(`startup · interval=${Math.round(INTERVAL_MS / 1000)}s`);
